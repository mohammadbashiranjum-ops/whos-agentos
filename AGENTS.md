...
- **Background runs are durable** (`queue=QueueConfig(durable=True)` in `app/main.py`). A `background=true` run is accepted as a committed row in the AgentOS database, so it survives a redeploy, and an `Idempotency-Key` re-POST within 24h returns the original run instead of starting a second one. WHOS's Option-A adapter (`whos-backend` `tools/whos_agentos_execution_adapter.py`) depends on that: it re-offers the same claim with the same key when a run outlives its read-back window. Without the durable queue agno ignores the key and each re-offer is a new provider run. `max_attempts` stays 1, so a crashed run fails visibly instead of silently re-running.
