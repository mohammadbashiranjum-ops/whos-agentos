...
- `app.settings.default_model()` picks the core model in one place, first match wins: the WHOS fuel endpoint when `WHOS_FUEL_BASE_URL` is set (`OpenAILike`, model alias `whos-work` — the WHOS Zero Platform LiteLLM router, which climbs from its free tier on its own), else Groq when `GROQ_API_KEY` is set (`openai/gpt-oss-120b`), else `OpenAIResponses(id="gpt-5.6")`. With `WHOS_FUEL_BASE_URL` unset, behaviour is exactly Groq-then-OpenAI. The fuel lane never sends `None` as its key — an OpenAI-compatible client with no key falls back to `OPENAI_API_KEY` and would carry it to the fuel host — so a keyless router gets an explicit placeholder instead. It routes chat models only: knowledge embeddings and the registry's media tools still use `OPENAI_API_KEY`.
...
|---|---|---|---|
| `OPENAI_API_KEY` | yes | — | OpenAI key for models + embeddings. Embeddings and media tools need it even when a fuel lane serves the chat model. |
| `WHOS_FUEL_BASE_URL` | no | — | OpenAI-compatible WHOS Zero Platform endpoint (e.g. `http://127.0.0.1:4000/v1`). Set, it becomes the core chat model ahead of Groq and OpenAI; unset, nothing changes. |
| `WHOS_FUEL_API_KEY` | no | — | Bearer key for the fuel endpoint (the LiteLLM master/virtual key). Unset sends a fixed placeholder, never `OPENAI_API_KEY`. |
| `AGENTOS_WHOS_FUEL_MODEL` | no | `whos-work` | Model alias requested from the fuel endpoint. |
| `GROQ_API_KEY` | no | — | Groq lane; used for the core chat model when set and no WHOS fuel endpoint is configured. |
| `AGENTOS_GROQ_MODEL` / `AGENTOS_OPENAI_MODEL` | no | `openai/gpt-oss-120b` / `gpt-5.6` | Model id override for the Groq / OpenAI lane. |
